...
class MessageController extends Controller
{
    ...
    public function index()
    {
        ...
        $users = DB::select("SELECT DISTINCT users.id, users.name, users.image, users.email, count(is_read) as unread FROM users INNER  JOIN  messages ON users." . $my_id . " = messages.from OR is_read = 0 OR messages.to = users." . $my_id . " WHERE users.id != " . $my_id . " GROUP BY users.id, users.name, users.email");
